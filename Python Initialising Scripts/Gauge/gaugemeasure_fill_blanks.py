...
class ReadInNimrod:
    def __init__(self, startdatetime, enddatetime, measure):
        self.station = measure
        try:
    ...
    def fill_in_blanks(self):
        while self.startdatetime.time() <= self.enddatetime.time() and self.startdatetime.date() <= self.enddatetime.date():
            self.cur.execute("SELECT value FROM hourlygaugereading WHERE measureid = (%s) AND datetime = (%s)",(self.station,self.startdatetime,))
            ...
            self.startdatetime += datetime.timedelta(minutes=15)


obj = ReadInNimrod('01-01-2017 00:00', '31-01-2017 23:45', '2133-level-stage-i-15_min-mASD')
